<x-app-layout>
    <x-card-container page_title="Manajemen Sub Service">
        <div
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-6 text-center">
                    ...
                    <h3 class="mb-5 text-lg font-normal text-gray-500">
                        Apakah kamu yakin ingin menghapus Sub Service ini?
                    </h3>
    ...
    @push('js-internal')
        <script>
            function btnEdit(id) {
                let url = "{{ route('admin.sub-service.update', ':id') }}".replace(':id', id);
                $('#create-modal form').attr('action', url);
                $('#create-modal form').trigger('reset');
                $('#create-modal #modal-title').text('Formulir Edit Sub Service');

                $.ajax({
                    url: "{{ route('admin.sub-service.get-by-id') }}",
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: id
            ...
            function btnDelete(id) {
                $('#delete-modal form').attr('action', "{{ route('admin.sub-service.destroy', ':id') }}".replace(':id', id));
            }
            $(function() {
                $('#btnAdd').click(function(e) {
                    e.preventDefault();
                    $('#create-modal form').attr('action',
                        "{{ route('admin.sub-service.store', ':serviceId') }}".replace(':serviceId',
                            '{{ $serviceId }}'));
                    $('#create-modal form').trigger('reset');
                    $('#create-modal #modal-title').text('Formulir Tambah Sub Service');
                });
