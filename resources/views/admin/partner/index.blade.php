<x-app-layout>
    <x-card-container page_title="Manajemen Partner">
        <div
    ...
    @push('js-internal')
        <script>
            function btnEdit(id) {
                let url = "{{ route('admin.partner.update', ':id') }}".replace(':id', id);
                $('#create-modal form').attr('action', url);
                $('#create-modal form').trigger('reset');
                $('#create-modal #modal-title').text('Formulir Edit Partner');

                $.ajax({
                    url: "{{ route('admin.partner.get-by-id') }}",
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: id
            ...
            function btnDelete(id) {
                $('#delete-modal form').attr('action', "{{ route('admin.partner.destroy', ':id') }}".replace(':id', id));
            }
            $(function() {
                $('#btnAdd').click(function(e) {
                    e.preventDefault();
                    $('#create-modal form').attr('action', "{{ route('admin.partner.store') }}");
                    $('#create-modal form').trigger('reset');
                    $('#create-modal #modal-title').text('Formulir Tambah Partner');
                });
