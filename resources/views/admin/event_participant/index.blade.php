<x-app-layout>
    <x-card-container page_title="Manajemen Peserta Acara">
        <div
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left text-gray-500">
                ...
                <tbody>
                    @forelse ($eventParticipants as $data)
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <form action="" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <x-select id="event_id" name="event_id" label="Nama Acara" required>
                        ...
                        <x-input id="email" name="email" label="Email" type="email" required />

                        <x-input id="phone" name="phone" label="No. Telepon" type="number" required />
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-6 text-center">
                    ...
                    <h3 class="mb-5 text-lg font-normal text-gray-500">
                        Apakah kamu yakin ingin menghapus Peserta Acara ini?
                    </h3>
    ...
    @push('js-internal')
        <script>
            function btnEdit(id) {
                let url = "{{ route('admin.event-participant.update', ':id') }}".replace(':id', id);
                $('#create-modal form').attr('action', url);
                $('#create-modal form').trigger('reset');
                $('#create-modal #modal-title').text('Formulir Edit Peserta Acara');

                $.ajax({
                    url: "{{ route('admin.event-participant.get-by-id') }}",
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: id
                    ...
                    success: function(data) {
                        $('#create-modal #event_id').val(data.event_id);
                        $('#create-modal #name').val(data.name);
                        $('#create-modal #email').val(data.email);
            ...
            function btnDelete(id) {
                $('#delete-modal form').attr('action', "{{ route('admin.event-participant.destroy', ':id') }}".replace(':id',
                    id));
            }
            $(function() {
                $('#btnAdd').click(function(e) {
                    e.preventDefault();
                    $('#create-modal form').attr('action', "{{ route('admin.event-participant.store') }}");
                    $('#create-modal form').trigger('reset');
                    $('#create-modal #modal-title').text('Formulir Tambah Peserta Acara');
                });
