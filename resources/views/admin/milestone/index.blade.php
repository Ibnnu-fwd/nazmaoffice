<x-app-layout>
    <x-card-container page_title="Manajemen Milestone">
        <div
    ...
    @push('js-internal')
        <script>
            function btnEdit(id) {
                let url = "{{ route('admin.milestone.update', ':id') }}".replace(':id', id);
                $('#create-modal form').attr('action', url);
                $('#create-modal form').trigger('reset');
                $('#create-modal #modal-title').text('Formulir Edit Milestone');

                $.ajax({
                    url: "{{ route('admin.milestone.get-by-id') }}",
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: id
                    ...
                    success: function(data) {
                        $('#create-modal #year').val(data.year);
            ...
            function btnDelete(id) {
                $('#delete-modal form').attr('action', "{{ route('admin.milestone.destroy', ':id') }}".replace(':id', id));
            }
            $(function() {
                $('#btnAdd').click(function(e) {
                    e.preventDefault();
                    $('#create-modal form').attr('action', "{{ route('admin.milestone.store') }}");
                    $('#create-modal form').trigger('reset');
                    $('#create-modal #modal-title').text('Formulir Tambah Milestone');
                });
