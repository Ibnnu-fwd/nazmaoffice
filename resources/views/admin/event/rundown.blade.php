<x-app-layout>
    <x-card-container page_title="Manejemen Jadwal Rundown">
        <div class="mx-4 py-4">
            ...
            <table class="w-full text-sm text-left text-gray-500">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 ">
                    <tr>
                        ...
                        <th scope="col" class="p-4">Waktu Mulai</th>
                        <th scope="col" class="p-4">Waktu Berakhir</th>
                        <th scope="col" class="p-4">Pemateri</th>
                        <th scope="col" class="p-4">Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($rundowns as $data)
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <form action="" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <x-input id="title" name="title" type="text" label="Judul" required="true" />

                        <x-textarea id="description" name="description" label="Deskripsi" required="true" />

                        <x-input id="start_time" name="start_time" type="time" label="Waktu Mulai"
                            required="true" />

                        <x-input id="end_time" name="end_time" type="time" label="Waktu Selesai"
                            required="true" />

                        <x-input id="speaker" name="speaker" type="text" label="Pembicara" required="true" />
    ...
    @push('js-internal')
        <script>
            function btnEdit(id) {
                let url = "{{ route('admin.event.rundown.update', ':id') }}".replace(':id', id);
                $('#create-modal form').attr('action', url);
                $('#create-modal form').trigger('reset');
                $('#create-modal #modal-title').text('Formulir Edit Rundown');

                $.ajax({
                    url: "{{ route('admin.event.rundown.get-by-id') }}",
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: id
                    ...
                    success: function(data) {
                        ...
                        $('#create-modal #start_time').val(data.start_time);
                        $('#create-modal #end_time').val(data.end_time);
                        $('#create-modal #speaker').val(data.speaker);
                    },
                    error: function() {
                        Swal.fire({
                            icon: 'error',
            ...
            function btnDelete(id) {
                $('#delete-modal form').attr('action', "{{ route('admin.event.rundown.destroy', ':id') }}".replace(':id', id));
            }

            $(function() {
                $('#btnAddEvent').click(function(e) {
                    e.preventDefault();
                    $('#create-modal form').attr('action', "{{ route('admin.event.rundown.store', ':id') }}"
                        .replace(':id', '{{ $event->id }}'));
                    $('#create-modal form').trigger('reset');
                    $('#create-modal #modal-title').text('Formulir Tambah Rundown');
                });
