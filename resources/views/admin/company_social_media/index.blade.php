<x-app-layout>
    <x-card-container page_title="Manajemen Sosial Media">
        <div
            ...
            <table class="w-full text-sm text-left text-gray-500">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 ">
                    <tr>
                        ...
                        <th scope="col" class="p-4">Ikon</th>
                        <th scope="col" class="p-4">Link</th>
                        <th scope="col" class="p-4">Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($companySocialMedias as $data)
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <form action="" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <x-input id="icon" name="icon" label="Ikon" required />

                        <x-input id="link" name="link" label="Link" required />
    ...
    @push('js-internal')
        <script>
            function btnEdit(id) {
                let url = "{{ route('admin.company-social-media.update', ':id') }}".replace(':id', id);
                $('#create-modal form').attr('action', url);
                $('#create-modal form').trigger('reset');
                $('#create-modal #modal-title').text('Formulir Edit Sosial Media');

                $.ajax({
                    url: "{{ route('admin.company-social-media.get-by-id') }}",
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: id
                    ...
                    success: function(data) {
                        ...
                        $('#create-modal #icon').val(data.icon);
                        $('#create-modal #link').val(data.link);
                    },
                    error: function() {
                        Swal.fire({
                            icon: 'error',
            ...
            function btnDelete(id) {
                $('#delete-modal form').attr('action', "{{ route('admin.company-social-media.destroy', ':id') }}".replace(':id',
                    id));
            }
            $(function() {
                $('#btnAdd').click(function(e) {
                    e.preventDefault();
                    $('#create-modal form').attr('action', "{{ route('admin.company-social-media.store') }}");
                    $('#create-modal form').trigger('reset');
                    $('#create-modal #modal-title').text('Formulir Tambah Sosial Media');
                });
