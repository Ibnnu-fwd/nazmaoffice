<x-app-layout>
    <x-card-container page_title="Manajemen Testimoni">
        <div
                    ...
                    @forelse ($testimonials as $data)
                        <tr class="border-b hover:bg-gray-100 dark:hover:bg-gray-700">
                            ...
                            <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">
                                <img src="{{ asset('storage/testimonial/' . $data->profile_picture) }}"
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <form action="" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <x-input id="fullname" name="fullname" label="Nama" required />

                        <x-input-file id="profile_picture" name="profile_picture" label="Gambar" required />
                        ...
                        <x-textarea id="testimonial" name="testimonial" label="Testimoni" required />
    ...
    @push('js-internal')
        <script>
            function btnEdit(id) {
                let url = "{{ route('admin.testimonial.update', ':id') }}".replace(':id', id);
                $('#create-modal form').attr('action', url);
                $('#create-modal form').trigger('reset');
                $('#create-modal #modal-title').text('Formulir Edit Testimoni');

                $.ajax({
                    url: "{{ route('admin.testimonial.get-by-id') }}",
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: id
                    ...
                    success: function(data) {
                        ...
                        $('#create-modal #testimonial').val(data.testimonial);
                    },
                    error: function() {
                        Swal.fire({
                            icon: 'error',
            ...
            function btnDelete(id) {
                $('#delete-modal form').attr('action', "{{ route('admin.testimonial.destroy', ':id') }}".replace(':id', id));
            }
            $(function() {
