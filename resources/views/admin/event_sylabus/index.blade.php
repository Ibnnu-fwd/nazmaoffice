<x-app-layout>
    <x-card-container page_title="Manajemen Silabus">
        <div class="mx-4 py-4">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left text-gray-500">
                ...
                <tbody>
                    @forelse ($eventSylabuses as $data)
                ...
                <form action="" method="POST">
                    ...
                    <div class="p-6 space-y-6">
                        <x-input id="title" name="title" label="Judul" type="text"
                            placeholder="Masukan judul Silabus" required />
                    </div>
                    <!-- Modal footer -->
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-6 text-center">
                    ...
                    <h3 class="mb-5 text-lg font-normal text-gray-500">
                        Apakah kamu yakin ingin menghapus Silabus ini?
                    </h3>
                    <form action="" method="POST">
                        @csrf
    ...
    @push('js-internal')
        <script>
            function btnEdit(id) {
                ...
                let url = "{{ route('admin.sylabus.update', ':id') }}";
                url = url.replace(':id', id);
                $('form').attr('action', url);
                $('#modal-title').text('Formulir Edit Silabus');

                $.ajax({
                    url: "{{ route('admin.sylabus.get-by-id') }}",
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: id
            ...
            function btnDelete(id) {
                $('#delete-modal form').attr('action', "{{ route('admin.sylabus.destroy', ':id') }}".replace(':id', id));
            }
            ...
            $(function() {
                $('#btnAddCategory').click(function(e) {
                    e.preventDefault();
                    $('input[name="title"]').val('');
                    let url = "{{ route('admin.sylabus.store', ':eventId') }}";
                    url = url.replace(':eventId', '{{ $eventId }}');
                    $('form').attr('action', url);
                    $('#modal-title').text('Formulir Tambah Silabus');
                });
