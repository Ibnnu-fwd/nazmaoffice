<x-app-layout>
    <x-card-container page_title="Manajemen Manfaat Layanan">
        <div class="mx-4 py-4">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left text-gray-500">
                ...
                <tbody>
                    @forelse ($serviceBenefits as $data)
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="flex items-start justify-between p-4 border-b rounded-t">
                    <h3 class="text-lg font-medium text-gray-900" id="modal-title">
                        Formulir Tambah Manfaat Layanan
                    </h3>
                    <button type="button"
    ...
    @push('js-internal')
        <script>
            function btnEdit(id) {
                let url = "{{ route('admin.service.benefit.update', ':id') }}".replace(':id', id);
                $('#create-modal form').attr('action', url);
                $('#create-modal form').trigger('reset');
                $('#create-modal #modal-title').text('Formulir Edit Manfaat Layanan');

                $.ajax({
                    url: "{{ route('admin.service.benefit.get-by-id') }}",
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: id
            ...
            function btnDelete(id) {
                $('#delete-modal form').attr('action', "{{ route('admin.service.benefit.destroy', ':id') }}".replace(':id',
                    id));
            }
            $(function() {
                $('#btnAdd').click(function(e) {
                    ...
                    let url = "{{ route('admin.service.benefit.store', ':id') }}".replace(':id', serviceId);
                    $('#create-modal form').attr('action', url);
                    $('#create-modal form').trigger('reset');
                    $('#create-modal #modal-title').text('Formulir Tambah Manfaat Layanan');
                });
