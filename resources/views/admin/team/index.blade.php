<x-app-layout>
    <x-card-container page_title="Manajemen Tim">
        <div
                    ...
                    @forelse ($teams as $data)
                        <tr class="border-b hover:bg-gray-100 dark:hover:bg-gray-700">
                            ...
                            <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">
                                <img src="{{ asset('storage/teams/' . $data->image) }}"
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <form action="" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <x-input id="instagram" name="instagram" label="Instagram" />

                        <x-input id="twitter" name="twitter" label="Twitter" />

                        <x-input id="linkedin" name="linkedin" label="Linkedin" />
    ...
    @push('js-internal')
        <script>
            function btnEdit(id) {
                let url = "{{ route('admin.team.update', ':id') }}".replace(':id', id);
                $('#create-modal form').attr('action', url);
                $('#create-modal form').trigger('reset');
                $('#create-modal #modal-title').text('Formulir Edit Tim');

                $.ajax({
                    url: "{{ route('admin.team.get-by-id') }}",
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: id
            ...
            function btnDelete(id) {
                $('#delete-modal form').attr('action', "{{ route('admin.team.destroy', ':id') }}".replace(':id', id));
            }
            $(function() {
