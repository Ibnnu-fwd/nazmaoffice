<x-app-layout>
    <x-card-container page_title="Manajemen Alamat">
        <div
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left text-gray-500">
                ...
                <tbody>
                    @forelse ($companyAddresses as $data)
                        <tr class="border-b hover:bg-gray-100 dark:hover:bg-gray-700">
                            ...
                            <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">
                                <a href="{{ $data->link }}" class="hover:text-primary">
                                    Kunjungi Link
                                </a>
                            </td>
                            <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">
    ...
    @push('js-internal')
        <script>
            function btnEdit(id) {
                let url = "{{ route('admin.company-address.update', ':id') }}".replace(':id', id);
                $('#create-modal form').attr('action', url);
                $('#create-modal form').trigger('reset');
                $('#create-modal #modal-title').text('Formulir Edit Alamat');

                $.ajax({
                    url: "{{ route('admin.company-address.get-by-id') }}",
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: id
                    ...
                    success: function(data) {
                        ...
                        $('#create-modal #address').val(data.address);
            ...
            function btnDelete(id) {
                $('#delete-modal form').attr('action', "{{ route('admin.company-address.destroy', ':id') }}".replace(':id',
                    id));
            }
            $(function() {
                $('#btnAdd').click(function(e) {
                    e.preventDefault();
                    $('#create-modal form').attr('action', "{{ route('admin.company-address.store') }}");
                    $('#create-modal form').trigger('reset');
                    $('#create-modal #modal-title').text('Formulir Tambah Alamat');
                });
